...
%% @doc add token bucket in ets
-spec add_token_bucket(token_bucket()) -> atom().
add_token_bucket(TokenBucket) ->
    ets:insert(token_buckets, TokenBucket).

%% @doc update token bucket in ets
-spec update_token_bucket(token_bucket()) -> atom().
update_token_bucket(TokenBucket) ->
    ets:insert(token_buckets, TokenBucket).
...
check_token_bucket_in_ets(Dataflow, BucketName) ->
    ...
    update_token_bucket(NewTokenBucket),
    Pause.
